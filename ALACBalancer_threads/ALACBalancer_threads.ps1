<# Start Functions #>
function ConvertTo-ALACDirectory($folderPath, $targetDecibels = "-18", $threshold = ".25", $maxPeak = "-.01", $logPath = "$($folderPath)\ALACBalancer.log", [switch]$unluckyMode, $throttleLimit = 4) {
    ...
    if (Test-Path -LiteralPath $folderPath) {
        ...
        if ($files) {
            ...
            New-ALACLog -logPath $logPath -message ">>Processing files with $throttleLimit parallel threads"
            
            # Use ForEach-Object -Parallel for multithreaded processing
            $files | ForEach-Object -Parallel {
                $file = $_
                $targetDecibels = $using:targetDecibels
                $threshold = $using:threshold
                $maxPeak = $using:maxPeak
                $logPath = $using:logPath
                $unluckyMode = $using:unluckyMode
                
                # Inline the conversion logic in the parallel block
                $threadGuid = [System.Guid]::NewGuid().ToString().Substring(0, 8)
                $tempFile = "$($env:LOCALAPPDATA)`\Temp\ConvertTo-LeveledALAC-$threadGuid\$($file.Name)"
                Write-Host -ForegroundColor Yellow "Processing: $($file.Name) [Thread: $([System.Threading.Thread]::CurrentThread.ManagedThreadId)]"
                
                    if (Test-Path -LiteralPath $file.FullName -ErrorAction SilentlyContinue) {
                    New-Item -Path "$($env:LOCALAPPDATA)`\Temp" -Name "ConvertTo-LeveledALAC-$threadGuid" -ItemType Directory -Force -ErrorAction SilentlyContinue | Out-Null
                    Copy-Item -LiteralPath $file.FullName -Destination $tempFile -Force -Confirm:$false -ErrorAction SilentlyContinue
                    
                    # Get loudness data
                    $stderr = & $env:ffmpeg -y -i "$tempFile" -af "loudnorm=I=-18:TP=-.01:LRA=1:print_format=json" -f null - 2>&1
                    $jsonMatch = [regex]::Match($stderr, '\{\s*"input_i"[\s\S]*?"target_offset"\s*:\s*"[^"]*"\s*\}')
                    
                    if ($jsonMatch.Success) {
                        try {
                            $jsonData = $jsonMatch.Value | ConvertFrom-Json
                            $data = @{
                                input_i = [double]$jsonData.input_i
                                input_tp = [double]$jsonData.input_tp
                                input_lra = [double]$jsonData.input_lra
                                input_thresh = [double]$jsonData.input_thresh
                                output_i = [double]$jsonData.output_i
                                output_tp = [double]$jsonData.output_tp
                                output_lra = [double]$jsonData.output_lra
                                output_thresh = [double]$jsonData.output_thresh
                                normalization_type = $jsonData.normalization_type
                                target_offset = [double]$jsonData.target_offset
                            }
                            $data.DR = $data.input_tp - $data.input_i
                            
                            Write-Host -ForegroundColor Cyan "  Input: $($data.input_i)dB | TP: $($data.input_tp)dB"
                            ...
                            if ($data) {
                                ...
                                if (([math]::abs($volAdjust) -gt [math]::abs($threshold)) -or (!([io.path]::GetExtension($file.FullName) -in '.m4a'))) {
                                    if ($data.input_tp -gt -40) {
                                        Write-Host -ForegroundColor Green "  ✓ Adjusting by $($volAdjust)dB"
                                        
                                        $tempPath = "$($tempFile | Split-Path -Parent)\temp_$($file.Name)"
                                        $outputPath = "$($tempFile | Split-Path -Parent)\output_$($file.Name)"
                                        
                                        Move-Item -LiteralPath $tempFile -Destination $tempPath -Force
                                        $arg2 = ' -y -i "{0}" -filter:a "volume={1}dB" -c:v copy -map_metadata:s:a 0:s:a -acodec alac -ar 44100 -sample_fmt s16p "{2}"' -f $tempPath, $volAdjust, $outputPath
                                        Start-Process -FilePath $env:ffmpeg -ArgumentList $arg2 -NoNewWindow -Wait
                                        
                                        if (Test-Path -LiteralPath $outputPath) {
                                            if ($unluckyMode) {
                                                $backupPath = "$($file.FullName | Split-Path -Parent)\(backup) $($file.Name)"
                                                Move-Item -LiteralPath $file.FullName -Destination $backupPath -Force -Confirm:$false -ErrorAction SilentlyContinue
                                            }
                                            else {
                                                Remove-Item -LiteralPath $file.FullName -Force -Confirm:$false -ErrorAction SilentlyContinue
                                            }
                                            Move-Item -LiteralPath $outputPath -Destination $file.FullName -Force -Confirm:$false
                                        }
                                        Remove-Item -LiteralPath $tempPath -Confirm:$false -ErrorAction SilentlyContinue
                                    }
                                    else {
                                        Write-Host -ForegroundColor DarkYellow "  ⊘ Skipping (silent file)"
                                        Remove-Item -LiteralPath $tempFile -Force -Confirm:$false -ErrorAction SilentlyContinue
                                    }
                                }
                                else {
                                    Write-Host -ForegroundColor DarkYellow "  ⊘ Within threshold"
                                    Remove-Item -LiteralPath $tempFile -Force -Confirm:$false -ErrorAction SilentlyContinue
                                }
                            }
                        }
                        ...
                        Write-Host -ForegroundColor Red "  ✗ No JSON found in ffmpeg output"
                        Remove-Item -LiteralPath $tempFile -Force -Confirm:$false -ErrorAction SilentlyContinue
                    }
                }
                else {
                    Write-Host -ForegroundColor Red "  ✗ File not found: $($file.FullName)"
                }
...
function ConvertTo-LeveledALAC($filePath, $targetDecibels = "-18", $threshold = ".25", $maxPeak = "-.01", $logPath = "$($filePath)`.log", [switch]$unluckyMode) {
    # Create a unique temp folder per thread to avoid collisions with parallel processing
    $threadGuid = [System.Guid]::NewGuid().ToString().Substring(0, 8)
    $tempFile = "$($env:LOCALAPPDATA)`\Temp\ConvertTo-LeveledALAC-$threadGuid\$($filePath | Split-Path -Leaf)"
    New-ALACLog -logPath $logPath -message "Processing file ""$($filePath)"" [Temp: ConvertTo-LeveledALAC-$threadGuid]"
